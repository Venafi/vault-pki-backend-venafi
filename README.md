...
We added capability in order to store by hash, which will be the ID to read the certificate from storage.
The hash is generated by:

`Common Name + SAN DNS + Zone`

At least, is required, to set any of: `Common Name` or `SAN DNS`.
